...
//				}
	...
	public boolean requestSG(PatientBmiView patientBmiView) throws Exception {
		...
		String url="http://172.1.2.117:9071/measure/api/patSignSubmit";
		...
		patdata.setMeasureOrgId("42482523300");
	...
	/*public boolean requestBP(List<BloodPressure> bplist) throws Exception {
		...
		String url = "http://172.1.2.117:9071/measure/api/chronic/submit";
		Pwdbpdata bpdata = new Pwdbpdata();

		BloodPressure bp = bplist.get(0);
		
		//System.out.println(String.valueOf(bp.getIdCard()) + "||" + bplist.size() + "||" + bplist.get(0).getId()+ "-" + bplist.get(bplist.size()-1).getId());
		...
		if (bplist.size() == 2) {
			...
			// if (bloodPressure.getIsAverage().equals(0)){
