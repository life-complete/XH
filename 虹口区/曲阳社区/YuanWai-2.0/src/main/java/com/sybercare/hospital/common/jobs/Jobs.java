...
public class Jobs {
	...
	public void removeLog() {
		try {
			delLog("D:/Hesicare/Opt/java项目/logs/");
		}catch (Exception e)
		{
			e.printStackTrace();
		}
	...
	public void doBmiIt() {
			try{
				...
				for (PatientBmiView patientBmiView : bmiview) {
					printwrites("D:/Hesicare/Opt/java项目/logs/sg/"+patientBmiView.getDeptid()+"/"+new SimpleDateFormat("yyyy-MM-dd").format(new Date()) +".txt","job 身高定时器：" + Constants.convert(new Date(), Constants.format2));
	...
	public void doXueTangIt() {
		try {
			...
			for (BloodGlucose bloodGlucose : bloodGlucoses1) {
				printwrites("D:/Hesicare/Opt/java项目/logs/xt/"+bloodGlucose.getDeptid()+"/"+new SimpleDateFormat("yyyy-MM-dd").format(new Date()) +".txt","job 血糖定时器：" + Constants.convert(new Date(), Constants.format2));
	...
	public void doXueYaIt() {
		try {
			...
			while (it.hasNext()) {
				...
				printwrites("D:/Hesicare/Opt/java项目/logs/xy/"+bplist.get(0).getDeptid()+"/"+new SimpleDateFormat("yyyy-MM-dd").format(new Date()) +".txt","job 血压定时器：" + Constants.convert(new Date(), Constants.format2));
				...
				boolean result=requestBP(bplist,url,comcode,name,resultname,resultnum);
				if (result) {
	...
	public boolean requestSG(PatientBmiView patientBmiView,String url,String comcode,String name) throws Exception {
		...
		printwrites("D:/Hesicare/Opt/java项目/logs/sg/"+patientBmiView.getDeptid()+"/"+new SimpleDateFormat("yyyy-MM-dd").format(new Date()) +".txt",name+"-请求数据：" + json.toString());
		...
		requestXT=true;
		printwrites("D:/Hesicare/Opt/java项目/logs/sg/"+patientBmiView.getDeptid()+"/"+new SimpleDateFormat("yyyy-MM-dd").format(new Date()) +".txt",name+"-"+result);
		return requestXT;
	}
	public boolean requestXT(BloodGlucose bloodGlucose,String url,String comcode,String name) throws Exception {
		...
		printwrites("D:/Hesicare/Opt/java项目/logs/xt/"+bloodGlucose.getDeptid()+"/"+new SimpleDateFormat("yyyy-MM-dd").format(new Date()) +".txt",name+"-请求数据：" + json.toString());
		String header = InterfaceEnCode.getInterfaceKey();
		map.put("param", InterfaceEnCode.encrypt(json.toString()));
		HttpClientUtil util = new HttpClientUtil();
		String result = util.doPost(header, url, map);
		...
		requestXT=true;
		printwrites("D:/Hesicare/Opt/java项目/logs/xt/"+bloodGlucose.getDeptid()+"/"+new SimpleDateFormat("yyyy-MM-dd").format(new Date()) +".txt",name+"-"+"返回结果：" + result);
	...
	public boolean requestBPJY(List<BloodPressure> bloodPressure ,String url,String comcode,String name,String resultname,String resultnum) throws Exception {
		...
		jsonConfig.setJsonPropertyFilter(filter);
		JSONObject json = JSONObject.fromObject(patdata, jsonConfig);
		printwrites("D:/Hesicare/Opt/java项目/logs/xy/"+bloodPressure.get(0).getDeptid()+"/"+new SimpleDateFormat("yyyy-MM-dd").format(new Date()) +".txt",name+"-请求数据：" + json.toString());
		String header = InterfaceEnCode.getInterfaceKey();
		map.put("param", InterfaceEnCode.encrypt(json.toString()));
		HttpClientUtil util = new HttpClientUtil();
		String result = util.doPost(header, url, map);
		...
		printwrites("D:/Hesicare/Opt/java项目/logs/xy/"+bloodPressure.get(0).getDeptid()+"/"+new SimpleDateFormat("yyyy-MM-dd").format(new Date()) +".txt",name+"-"+"返回结果：" + result);
		return requestBP;
	...
	public boolean requestBP(List<BloodPressure> bplist,String url,String comcode,String name,String resultname,String resultnum) throws Exception {
		...
		printwrites("D:/Hesicare/Opt/java项目/logs/xy/"+bplist.get(0).getDeptid()+"/"+new SimpleDateFormat("yyyy-MM-dd").format(new Date()) +".txt",name+"-请求数据：" + json.toString());
		String result = this.sendHttpPost(url, json.toString());
		JSONObject resultJson = JSONObject.fromObject(result);
		printwrites("D:/Hesicare/Opt/java项目/logs/xy/"+bplist.get(0).getDeptid()+"/"+new SimpleDateFormat("yyyy-MM-dd").format(new Date()) +".txt",name+"-"+"返回结果：" + result);
	...
	public HospitalEnum info(String comcode){
		...
		if(comcode.equals("1001005")){hospitalEnum=HospitalEnum.hk_lc;}
		if(comcode.equals("1001004")){hospitalEnum=HospitalEnum.hk_qy;}
		return  hospitalEnum;
	}
}